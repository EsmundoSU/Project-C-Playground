...
{: .callout .note} NOTE: `SUCCEED()` is purely documentary and currently doesn't generate any user-visible output.
However, we may add `SUCCEED()` messages to googletest's output in the future.
...
{: .callout .note} NOTE: you can only use `FAIL()` in functions that return `void`. See the
...
[a library of matchers](gmock_cheat_sheet.md#MatcherList) for validating arguments passed to mock objects. A gMock *
...
gMock provides, read
[this](gmock_cook_book.md##using-matchers). It's easy to write your [own matchers](gmock_cook_book.md#NewMatchers) too.
